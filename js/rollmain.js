...
//자바스크립트 함수 선언
function play_w(directw){ //다이렉트는 슬라이드 좌우방향 지정하는 매개변수
    if(directw=="right"){
        wnum = wnum+1;
        if(wnum>2) {wnum=0;}
    }
    ...
    $(".rollingbtn").find('li.seq a').each(function(){
        //슬라이드 이미지 아래의 이동버튼을 초기값으로 동그란모양으로 만든다.
